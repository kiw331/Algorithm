...
# 그래프를 인접행렬로 표현하면 메모리제한 넘어갈확률이 큼(최대 4*10^8) - 대신 시작 정점 별로 간선을 묶어준다.
graph = [[] for _ in range(v+1)]
for edge in uvw:
    graph[edge[0]].append(edge[1:3])
    
# # 간선 정보 입력 -> 이렇게 입력하는게 더 효율적일듯
# for _ in range(e):
...
dis = [maxsize for _ in range(v+1)]   # d[3]: 시작 정점에서 정점3까지 최단거리. 처음에 무한(최대값+1)로 초기화
dis[k] = 0
...
for i in dis[1:]:   # [1]부터 출력해서 범위 맞춤
